// src/Receipt.tsx - Enhanced version with improved mobile responsiveness
...
const Receipt: React.FC<ReceiptProps> = ({ token }) => {
  ...
  const downloadImage = async () => {
    ...
    try {
      ...
      link.download = `${userName}-receiptify-${timeRange}.png`;
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <Music className="h-5 w-5 sm:h-6 sm:w-6 text-green-400" />
              <span className="text-lg sm:text-xl font-bold text-white">Receiptify</span>
              <span className="hidden xs:inline text-sm text-gray-400">• Tracks</span>
            </div>
            <button
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors ${
                          showAlbumArt ? 'bg-green-500' : 'bg-gray-600'
                        }`}
                      >
                        <span
                          className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${
                            showAlbumArt ? 'translate-x-6' : 'translate-x-1'
                          }`}
                        />
                      </button>
                    </div>
                  ...
                  <div className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
                    {tracks.length > 0 ? (
                      tracks.map((track, index) => (
                        <div key={track.id} className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm">
                          ...
                          <div className="flex-grow min-w-0">
                            <p className="font-bold uppercase truncate leading-tight">{track.name}</p>
                            <p className="opacity-70 uppercase text-xs leading-tight truncate">
                      ...
                      <div className="text-center py-6 sm:py-8">
                        ...
                        <p className="opacity-50 text-xs sm:text-sm">Loading tracks...</p>
                      </div>
                    )}
                  </div>
