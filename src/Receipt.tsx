// src/Receipt.tsx - Enhanced version with modern UI/UX
                  ...
                  <div className="space-y-3 mb-6">
                    {tracks.length > 0 ? (
                      tracks.map((track, index) => (
                        <div key={track.id} className="flex items-center gap-3 text-sm">
                          ...
                          <div className="flex-grow min-w-0">
                            <p className="font-bold uppercase truncate">{track.name}</p>
                            <p className="opacity-70 uppercase text-xs truncate">
                      ...
                      <div className="text-center py-8">
                        ...
                        <p className="opacity-50">Loading tracks...</p>
                      </div>
                    )}
                  </div>
                          ...
                          <div className="grid grid-cols-3 gap-1">
                            {(['clean', 'torn', 'stacked'] as PaperEffect[]).map(effect => (
                              <button
                                ...
                                className={`py-2 px-3 rounded-lg text-xs font-medium transition-colors capitalize ${paperEffect === effect ? 'bg-green-500 text-white' : 'bg-white/10 text-gray-300 hover:bg-white/20'}`}
                              >
                                {effect}
                              </button>
                            ))}
                      ...
                      <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-3 gap-1">
                            {(['compact', 'standard', 'large'] as ReceiptSize[]).map(size => (
                              <button
                                ...
                                className={`py-2 px-3 rounded-lg text-xs font-medium transition-colors capitalize ${receiptSize === size ? 'bg-green-500 text-white' : 'bg-white/10 text-gray-300 hover:bg-white/20'}`}
                              >
                                {size}
                              </button>
                            ))}
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            ...
                            <label className="text-sm text-white">Show Album Art</label>
                          </div>
                          <button
                            onClick={() => setShowAlbumArt(!showAlbumArt)}
                            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${showAlbumArt ? 'bg-green-500' : 'bg-gray-600'}`}
                          >
                            <span
                              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${showAlbumArt ? 'translate-x-6' : 'translate-x-1'}`}
                            />
                          </button>
                        </div>
                      </div>
