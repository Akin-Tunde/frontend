// src/GenreReceipt.tsx - Enhanced version with receipt UI/UX
  ...
  useEffect(() => {
    const fetchDataAndProcessGenres = async () => {
      ...
      try {
        ...
        setError("Failed to calculate top genres. The Spotify token may have expired. Please try logging in again.");
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <Tags className="h-5 w-5 sm:h-6 sm:w-6 text-green-400" />
              <span className="text-lg sm:text-xl font-bold text-white">Receiptify</span>
              <span className="hidden xs:inline text-sm text-gray-400">• Genres</span>
            </div>
            <button
          ...
          <div className={showCustomization ? 'lg:col-span-2' : 'w-full'}>
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-xl sm:rounded-2xl p-4 sm:p-6 mb-6 sm:mb-8 border border-white/20">
              ...
              <div className="text-center bg-white/5 rounded-lg py-3">
                <p className="font-medium text-white text-sm">ALL TIME</p>
                <p className="text-xs text-gray-400 mt-1">Genre data is based on your all-time top artists</p>
              </div>
            </div>
              ...
              >
                ...
                >
                  <div className="text-center mb-4 sm:mb-6">
                    ...
                    <p className="text-xs sm:text-sm opacity-80">BASED ON ALL-TIME LISTENING</p>
                  ...
                  <div className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
                    {genres.slice(0, genreLimit).map((genre, index) => (
                      <div key={genre.name} className="flex justify-between items-center text-xs sm:text-sm">
                        <div className="flex items-center min-w-0">
                          <span className="font-bold w-5 sm:w-6 text-center flex-shrink-0">{index + 1}.</span>
                          <p className="font-bold uppercase truncate ml-2 sm:ml-3">{genre.name}</p>
                        </div>
                  ...
                  <div className="text-center pt-3 sm:pt-4 border-t border-current border-opacity-20">
                    ...
                    <div className="mt-1 sm:mt-2 text-xs opacity-60">{new Date().toLocaleDateString()}</div>
                  </div>
                </div>
              </div>
            </div>
